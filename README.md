...
## Learning Path
4~8이 꼭 필수적이라고 할 수는 없지만, 프로젝트를 진행하면서 알아두면 매우 유용합니다.
프로젝트 상황에 따라 우선순위를 정하자면:
...
ff-merge (4번) - 브랜치를 사용하기 시작하면 필요해집니다만, 초기에는 덜 중요할 수 있습니다.
amend (6번) - 마지막 커밋을 수정하는 기능으로, 유용하지만 필수는 아닙니다.

이미 1, 2, 3을 익히셨다면, 일단 8번(ignore)을 배우고 프로젝트를 시작하세요. 나머지는 필요할 때마다 배워가는 방식으로 진행해도 충분합니다.
